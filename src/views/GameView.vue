...
<template>
    <div class="game-container">
        ...
        <div class="status">
          ...
          <template v-else>
            <div class="current-player">
              Ход: <span :class="currentPlayer">{{ currentPlayer }}</span>
            </div>
          </template>
        </div>
        <div class="controls">
      ...
      <div class="game-board-container">
            ...
            >
              ...
              >
                <ChessPiece 
                  ...
                  :piece="piece" 
                  :style="pieceStyle"
                  :piece-style="settings.pieceStyle"
                />
              </div>
            </div>
          </div>
        ...
        <div class="move-tips" v-if="selectedPiece">
          <h3>Возможные ходы:</h3>
          <ul>
            <li v-for="(move, index) in availableMoves" :key="`move-${index}`">
              {{ getNotation(selectedPiece.position) }} → {{ getNotation(move) }}
            </li>
          </ul>
        </div>
      </div>
  ...
  // Вспомогательная функция для определения цвета клетки
  function isLightSquare(row, col) {
    return (row + col) % 2 === 0;
  }
  
  // Вспомогательные функции для проверки состояния клетки
  ...
  function isAvailableMove(row, col) {
    return availableMoves.value.some(([r, c]) => r === row && c === col);
  }
  ...
  function handleCellClick(row, col) {
    ...
    // Если уже есть выбранная фигура и клетка является доступным ходом
    if (selectedPiece.value && isAvailableMove(row, col)) {
  ...
  // Функция для получения шахматной нотации позиции
